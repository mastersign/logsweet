...
xterm -title "Log Broadcast A" -geometry 120x20 -e bash -c "cd $pdir && pipenv run python3 -m logsweet.cli broadcast -s -b 127.0.0.1:9501 \"$this_dir/a-*.log\"" &
xterm -title "Log Broadcast B" -geometry 120x20 -e bash -c "cd $pdir && pipenv run python3 -m logsweet.cli broadcast -s -b 127.0.0.1:9502 \"$this_dir/b-*.log\"" &
