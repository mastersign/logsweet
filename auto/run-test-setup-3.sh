...
xterm -title "Log Watch A" -geometry 120x10 -e bash -c \
    "cd \"$pdir\" && pipenv run python3 -m logsweet.cli watch -s -c 127.0.0.1:9504 \"$this_dir/test-3-a-*.log\"" &

xterm -title "Log Watch B" -geometry 120x10 -e bash -c \
    "cd \"$pdir\" && pipenv run python3 -m logsweet.cli watch -s -b 127.0.0.1:9505 \"$this_dir/test-3-b-*.log\"" &
