...
xterm -title "Log Mock" -geometry 120x10 -e bash -c \
    "cd \"$pdir\" && pipenv run python3 -m logsweet.cli mock -i 0.01 \"$this_dir/test-0.log\"" &

xterm -title "Log Watch" -geometry 120x10 -e bash -c \
    "cd \"$pdir\" && pipenv run python3 -m logsweet.cli watch -b 127.0.0.1:9500 \"$this_dir/test-0.log\"" &
