...
xterm -title "Log Watch A" -geometry 120x10 -e bash -c \
    "cd \"$pdir\" && pipenv run python3 -m logsweet.cli watch --echo -c 127.0.0.1:9503 -cfg \"$this_dir/demo.yml\" \"$this_dir/test-2-a-*.log\"" &

xterm -title "Log Watch B" -geometry 120x10 -e bash -c \
    "cd \"$pdir\" && pipenv run python3 -m logsweet.cli watch -c 127.0.0.1:9503 \"$this_dir/test-2-b-*.log\"" &
